...
class Data(object):
        ...
        if len(self.numList) > 0:
            try:
                ...
                for x in range(len(self.numList)):
                    self.numList[x].append(self.operationalVoltage[x])
    
                self.operationalPower = self.setOperationalPower()
                for x in range(len(self.numList)):
                    self.numList[x].append(self.operationalPower[x])
                self.DNE = False
            except ValueError:
                self.DNE = True
        else:
    ...
    def plot(self, write = 'n', op= 'n'):
        ...
        if op == 'y':
            plt.xlabel("Operational Voltage (V)", fontsize = 12)
        else:
...
def unifyHdf5(directory, saveLoc, overwrite = 'y', door=None):
                ...
                if len(x) == 2:
                    if isinstance(x[1], list):
                        try:
                            ...
                            if any(isinstance(i, list) for i in x[1]):
                                if isinstance(x[1][0], (int, float)):                                
                                    h.create_dataset(str(x[0]), data=np.array(x[1]))
                                else:
                                    for e in x[1]:
                                        h[e[0]] = e[1]
                        except:
...
def plotDevice( directory = "C:\Users\jye\Desktop", deviceName = "opv_Friday_d1",
               write = 'n', op = 'n'):
    import os
    import h5py
    import matplotlib.pyplot as plt
            ...
            if temp["Data"].attrs.get("Device Name") == deviceName:
                try:
                    if op == 'n':
                        ...
                        voltage = [y[2] for y in temp["Data"]["Plot Points"][:]]
    ...
    # label of axes
    fig.title('Aggregate J-V curve: ' + deviceName, fontsize = 25)
    plt.xlabel("Voltage(V)", fontsize = 12)
    plt.ylabel("Current (mA)", fontsize = 12)
...
def plotPowerHysteresis( directory, saveLoc, write = 'n', op = 'n'):
    ...
    for key in hdf5.keys():
        for key2 in hdf5[key].keys():
            try:            
                if op=='y':
                    op_power = hdf5[key][key2]['Plot Points'][3,:]
                    maxPower = max(op_power)
    ...
    # label of axes
    if op == 'n':
        title = 'Maximum Power vs. Time: ' + directory[(x+1):-5] 
    else:
        title = 'OP Maximum Power vs. Time: ' + directory[(x+1):-5] 
    plt.title(title + '\n', fontsize = 25)
    ...
    if write == 'y':
        try:
            if not os.path.exists(saveLoc):
                os.makedirs(saveLoc)
            plt.savefig(saveLoc  +'/' + title +'.jpg')
        except:
            print "Can't save graph"
    plt.show()

def plotvocHysteresis( directory, saveLoc, write = 'n', op = 'n'):
    from datetime import datetime
    import os
    import h5py
    import matplotlib.pyplot as plt
    ...
    for key in hdf5.keys():
        for key2 in hdf5[key].keys():
            try:
                if op=='y':
                    op_voltage = hdf5[key][key2]['Plot Points'][2,:]
                    voc = max(op_voltage)
                else:
    ...
    if op =='n':
        title = 'VOC vs. Time: ' + directory[(x+1):-5]
    else:
        title = 'OP VOC vs. Time: ' + directory[(x+1):-5]
    plt.title(title + '\n', fontsize = 25)
    plt.xlabel("Date", fontsize = 18)
...
def plotFillFactorHysteresis(directory, saveLoc, write = 'n', op='n'):
    from datetime import datetime
    import os
    import h5py
    import matplotlib.pyplot as plt
        ...
        for key2 in hdf5[key].keys():
            try:
                if op == 'n':
                    ...
                    oppower = hdf5[key][key2]["Plot Points"][3,:]
                    opmaxpower = max(oppower)
                    opvoltage = hdf5[key][key2]["Plot Points"][2,:]
                    isc = hdf5[key][key2]["Plot Points"][1,:]
                    opfillpower= [x*y for x,y in zip(opvoltage, isc)]
                    opmaxfillpower = max(opfillpower)
                    ff = opmaxpower/opmaxfillpower
            except:
                ff = None
            if ff != None:
                print "["+key+"]["+key2+"]"
    ...
    if op == 'n':
        title = 'Fill Factor vs. Time: ' + directory[(x+1):-5]
    else:
        title = 'OP Fill Factor vs. Time: ' + directory[(x+1):-5]
    # label of axes
    plt.title(title + '\n', fontsize=25)
    plt.xlabel("Date", fontsize=18)
...
def saveAllFigMerit(directory,saveloc, op='n'):

    plotiscHysteresis(directory, saveloc, write='y')
    plotvocHysteresis(directory, saveloc, write='y', op=op)
    plotPowerHysteresis(directory, saveloc, write='y', op=op)
    plotFillFactorHysteresis(directory, saveloc, write='y', op=op)
...
def plotAll(directory, saveloc, plot='y', refactor='y', op='n'):
    import os
    if not os.path.exists(saveloc):
        os.makedirs(saveloc)
    ...
    if plot =='y':
        for hdf5 in filedir:
            if hdf5[-5:] == '.hdf5':
                try:
                    saveAllFigMerit(saveloc + '/' + hdf5, saveloc + '/' + hdf5[:-5], op=op)
                except:
