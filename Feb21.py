...
def plotPowerHysteresis( directory, saveLoc, write = 'n'):
    ...
    axScatter.scatter(plotTimef, plotPowerf, color='B')
    axScatter.scatter(plotTimeb, plotPowerb, color='R')

    hdf5.close()
    x = directory.rfind('/')
    # label of axes
    plt.title('Maximum Power vs. Time: ' + directory[(x+1):-4] + '\n', fontsize = 25)
    ...
    if write == 'y':
        try:
            if not os.path.exists(saveLoc):
                os.makedirs(saveLoc)
            plt.savefig(saveLoc  +'/' +'Pmax v Time_' + directory[(x+1):-4] +'.jpg')
        except:
            print "Can't save graph"
    plt.show()
...
def plotvocHysteresis( directory, saveLoc, write = 'n'):
    ...
    axScatter.scatter(plotTimef, plotVocf, color='B')
    axScatter.scatter(plotTimeb, plotVocb, color='R')

    hdf5.close()
    x = directory.rfind('/')
    # label of axes
    plt.title('VOC vs. Time: ' + directory[(x+1):-4] + '\n', fontsize = 25)
    plt.xlabel("Date", fontsize = 18)
    ...
    if write == 'y':
        try:
            if not os.path.exists(saveLoc):
                os.makedirs(saveLoc)
            plt.savefig(saveLoc  +'/' +'VOC v Time_' + directory[(x+1):-4] +'.jpg')
        except:
            print "Can't save graph"
    plt.show()
...
def plotiscHysteresis( directory, saveLoc, write = 'n'):
    ...
    axScatter.scatter(plotTimef, plotIscf, color='B')
    axScatter.scatter(plotTimeb, plotIscb, color='R')

    hdf5.close()
    x = directory.rfind('/')
    # label of axes
    plt.title('ISC vs. Time: ' + directory[(x+1):-4] + '\n', fontsize=25)
    plt.xlabel("Date", fontsize = 18)
    ...
    if write == 'y':
        try:
            if not os.path.exists(saveLoc):
                os.makedirs(saveLoc)
            plt.savefig(saveLoc  +'/ISC v Time_' + directory[(x+1):-4] +'.jpg')
        except:
            print "Can't save graph"
    plt.show()
...
def plotFillFactorHysteresis(directory, saveLoc, write = 'n'):
    ...
    axScatter.scatter(plotTimeFor, plotFFFor, color='B')
    axScatter.scatter(plotTimeBac, plotFFBac, color='R')

    hdf5.close()
    x = directory.rfind('/')
    # label of axes
    plt.title('Fill Factor vs. Time: ' + directory[(x+1):-4] + '\n', fontsize=25)
    plt.xlabel("Date", fontsize=18)
    ...
    if write == 'y':
        try:
            if not os.path.exists(saveLoc):
                os.makedirs(saveLoc)
            plt.savefig(saveLoc + '\\Fill Factor v Time_' + directory[(x+1):-4] + '.jpg')
        except:
            print "Can't save graph"
    plt.show()
...
def plotAll(directory, saveloc):
    ...
    for hdf5 in filedir:
        if hdf5[-5:] == '.hdf5':
            try:
                saveAllFigMerit(saveloc + '/' + hdf5, saveloc + '/' + hdf5[:-4])
            except:
                print 'Wrong Hdf5 File'

plotAll('C:/Users/Eric/Desktop/J-V/test_oct12end', 'C:/Users/Eric/Desktop/Results/test_oct12end')
