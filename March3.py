...
def plotPowerHysteresis( directory, saveLoc, write = 'n', op = 'n'):
    ...
    for key in hdf5.keys():
        for key2 in hdf5[key].keys():
            try:            
                if op=='y':
                    op_power = hdf5[key][key2]['Attrributes']['Op Power'][:]
    ...
    if write == 'y':
        try:
            ...
            if op == 'n': 
                ...
                plt.savefig(saveLoc  +'/' +'Pmax v Time_'+ 'OP' + directory[(x+1):-5] +'.jpg')
        except:
            print "Can't save graph"
    plt.show()
...
def plotvocHysteresis( directory, saveLoc, write = 'n', op = 'n'):
    ...
    for key in hdf5.keys():
        for key2 in hdf5[key].keys():
            try:
                if op=='y':
                    op_voltage = hdf5[key][key2]['Attrributes']['Op Voltage'][:]
                    voc = max(op_voltage)
                else:
    ...
    if write == 'y':
        try:
            ...
            if op =='n':
                ...
                plt.savefig(saveLoc  +'/' +'VOC v Time_'+ 'OP' + directory[(x+1):-5] +'.jpg')                
        except:
            print "Can't save graph"
    plt.show()
