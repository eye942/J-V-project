...
def unifyHdf5(directory, saveLoc, overwrite = 'y'):
    ...
    for txtFile in listDir:
        if txtFile[-4:] == ".txt":
            ...
            name.parseFields()
            dName = name.parsedList[0][1]
            if name in nameDict:
                nameDict[dName]=nameDict[dName].append([name.numList, name.parsedList])
            else:
                nameDict[dName] = [[name.numList, name.parsedList]]        
    ...
    for key in nameDict:
        ...
        f = saveData.create_group('Data')
        for dataList in nameDict[key]:
            g = saveData.create_group('/Data/Attrbs')
            #dSetV = f.create_dataset("Voltage", np.array(self.voltage))
            #dSetC = f.create_dataset("Current", np.array(self.current))
            try:
                arr = np.array(nameDict[key][0])
                dSetPlot = f.create_dataset("Plot Points", data = arr)
            except:
                pass
            for x in nameDict[key][1]:
                #try:
                if len(x) == 2:
                    if isinstance(x[1], list):
                        h = g.create_group(x[0])
...
def plotDevice( directory = "C:\Users\jye\Desktop", deviceName = "opv_Friday_d1", write = 'n'):
    ...
    '''# now determine nice limits by hand:        
    axScatter.set_xlim(min(voltage) - min(np.absolute(voltage))  , max(voltage) + min(np.absolute(voltage)))
    axScatter.set_ylim(min(current) - min(np.absolute(current)) , max(current) + min(np.absolute(current)))
    '''
    # label of axes        
    fig.suptitle('Aggregate J-V curve: ' + deviceName, fontsize = 25)# + Data.parsedList[0])
    plt.xlabel("Voltage(V)", fontsize = 12)
    plt.ylabel("Current (mA)", fontsize = 12)
